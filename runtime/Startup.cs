...
public static class Startup
{
    ...
    /// <summary>Path of the core this image booted from (--asm / a .fasl core), or
    /// null when the core came from memory. Used only to identify the compiler
    /// generation that produced a .fasl.</summary>
    public static string? CorePath;

    private static string? _coreGeneration;
    private static bool _coreGenerationComputed;

    /// <summary>The core assembly this image booted from, when it booted from a
    /// .fasl PE. Null for the .sil text path. Carries the generation stamp.</summary>
    public static System.Reflection.Assembly? CoreAssembly;

    /// <summary>A short digest identifying the COMPILER a core embodies, or null when
    /// it cannot be determined. A .fasl records the value current when it was
    /// compiled; loading one whose value differs means its code was generated by a
    /// different compiler than the one now running, which is invisible otherwise —
    /// the fasl keeps the old code generation and a compiler fix appears to have no
    /// effect.
    ///
    /// The value is derived from the compiler SOURCE (the .sil the core was built
    /// from) and stamped into the core when it is written, NOT hashed from the core
    /// file at run time. Hashing the file was wrong twice over: sil-to-fasl gives the
    /// module a fresh GUID-suffixed name, so rebuilding a core from identical input
    /// changes its bytes; and crossgen2 rewrites the file again when producing the
    /// per-RID R2R core. Either one made every shipped .fasl look stale on a clean
    /// install. Reading a stamp the core carries survives both, because R2R
    /// compilation preserves managed metadata.</summary>
    /// <summary>Whether a thread can be started at all. Asked by running one:
    /// the browser's single-threaded build carries the whole threading API and
    /// refuses at Start, so nothing short of trying tells them apart.</summary>
    private static bool ThreadsStart()
    {
        try
        {
    ...
    public static string? CoreGeneration()
    {
        if (_coreGenerationComputed) return _coreGeneration;
        _coreGenerationComputed = true;
        try
        {
            if (CoreAssembly != null)
            {
                foreach (var a in System.Reflection.CustomAttributeExtensions
                             .GetCustomAttributes<System.Reflection.AssemblyMetadataAttribute>(CoreAssembly))
                    if (a.Key == CoreGenerationKey) { _coreGeneration = a.Value; break; }
            }
            // .sil text path (development `--asm`): the source itself is the
            // compiler, so hashing it is both meaningful and deterministic.
            else if (CorePath != null && System.IO.File.Exists(CorePath))
                _coreGeneration = HashFileGeneration(CorePath);
        }
        catch { /* best-effort: no stamp is better than a failed compile */ }
        return _coreGeneration;
    }

    /// <summary>The generation a core built from SOURCEPATH must carry: a short digest
    /// of the compiler source, so identical input always yields the same value.</summary>
    internal static string HashFileGeneration(string sourcePath)
    {
        using var sha = System.Security.Cryptography.SHA256.Create();
        using var fs = System.IO.File.OpenRead(sourcePath);
        return Compat.ToHexString(sha.ComputeHash(fs), 0, 8).ToLowerInvariant();
    }

    /// <summary>Assembly-metadata key holding the compiler generation. Written by the
    /// fasl writer (Emitter/FaslAssembler.cs) and read by the fasl loader
    /// (Runtime.Misc.cs). It lives here, not on the writer, because an emit-free
    /// build has no writer but still loads fasls — and must still be able to warn
    /// that one was compiled by a different core.</summary>
    internal const string CoreGenerationKey = "dotcl-core-generation";
    ...
    // Features list. Only what is true of the implementation wherever it runs
    // belongs here — a fasl carries these to the next machine. Anything true of
    // the machine or of this particular image is decided in Initialize().
    ...
    public static void Initialize()
    {
        ...
        if (Compat.IsBrowser())
        {
            // A page is none of the three above, and saying nothing is not a
            // neutral answer: UIOP's detect-os refuses to load at all unless one
            // of :unix / :windows / :genera / :mcl is present ("Congratulations
            // for trying ASDF on an operating system that is neither Unix, nor
            // Windows..."), which takes every asdf-built system down with it.
            //
            // :unix is what the rest of the runtime already assumes here — under
            // Emscripten the paths are '/'-rooted and every Compat.IsWindows
            // branch is false — so this makes *features* agree with the code
            // rather than introducing a new claim. :linux is deliberately not
            // added: there is no kernel to name.
            _features.Add("UNIX");
            // Without this the page would be indistinguishable from a Unix that
            // merely failed to say which one, and code has to be able to tell:
            // there are no subprocesses, no sockets, and no files but the ones
            // this session wrote.
            _features.Add("BROWSER");
        }

        // Matched by name, not against Architecture's members: this assembly is
        // built for netstandard2.0 as well, whose enum stops at Arm64, while the
        // runtime underneath may be a .NET that knows Wasm, S390x, Armv6 and the
        // rest. ToString() asks the enum that is actually running.
        //
        // Only the names portable Common Lisp already has a word for are pushed.
        // The others — Wasm, S390x, LoongArch64, Ppc64le, RiscV64 — are left
        // unsaid: none of them appears in any trivial-features backend, so any
        // keyword invented here would be one nobody reads, and claiming a wrong
        // one is worse. That was the state before this: everything that was not
        // Arm64 said X86-64, so a browser said it too, next to :32-bit.
        //
        // Armv6 folds into :arm. It is 32-bit ARM, which is all Lisp has a name
        // for, and the width is already carried by :32-bit below.
        switch (System.Runtime.InteropServices.RuntimeInformation
                    .ProcessArchitecture.ToString())
        {
            case "X86": _features.Add("X86"); break;
            case "X64": _features.Add("X86-64"); break;
            case "Arm": case "Armv6": _features.Add("ARM"); break;
            case "Arm64": _features.Add("ARM64"); break;
        }
        ...
        // Whether threads actually run here, rather than whether the code for
        // them is present. A single-threaded browser build has the API and
        // throws PlatformNotSupportedException on Start, so anything that read
        // :thread-support and took the threaded path — bordeaux-threads does —
        // failed at the first make-thread. Probed only where the answer is in
        // doubt; no desktop runtime lacks them.
        if (!Compat.IsBrowser() || ThreadsStart())
            _features.Add("THREAD-SUPPORT");

        // :DOTCL-EMIT says this build can generate code at run time. Absent on
        // netstandard2.0 / browser-wasm / -p:DotclNoEmit=true, where the tree-walk
        // interpreter is the only evaluator and COMPILE / COMPILE-FILE are
        // unavailable. Without it there was no way to ask that question from Lisp:
        // *EVALUATOR-MODE* answers :COMPILE on an emit-free build (nothing rebinds
        // it), so code — including the test harness's DEFTEST-COMPILED-ONLY —
        // could not tell the two apart.
#if DOTCL_EMIT
        _features.Add("DOTCL-EMIT");
#endif
    ...
    internal static void RegisterTernary(string name, Func<LispObject, LispObject, LispObject, LispObject> fn)
    {
        var lispFn = new LispFunction(args => {
            if (args.Length != 3)
                throw new LispErrorException(new LispProgramError($"{name}: wrong number of arguments: {args.Length} (expected 3)"));
    ...
    private static void RegisterDotclFunctions()
    {
        ...
        // dotcl:%handler-cluster-depth — (values handler-depth restart-depth). Exposes the
        // live HANDLER-BIND/HANDLER-CASE and RESTART-BIND/RESTART-CASE cluster-stack depths
        // so tests can assert clusters are balanced (see restart-cluster-leak regression).
        RegisterDotcl("%HANDLER-CLUSTER-DEPTH", new LispFunction(args => {
        ...
        RegisterDotcl("SIL-TO-FASL", new LispFunction(args => {
            ...
            // A core carries its own identity, not the identity of whatever core
            // built it: it IS this compiler source. Derived from the .sil so that
            // rebuilding from identical input keeps the value, and so the per-RID
            // R2R core (same metadata, different bytes) keeps it too.
            fasl.SelfGeneration = HashFileGeneration(inPath);
        ...
        RegisterDotcl("LAUNCH-PROCESS", new LispFunction(args => {
            ...
            LispObject directory = Nil.Instance, environment = Nil.Instance;
            ...
            System.Collections.Generic.List<string>? env = null;
            if (haveEnv)
            {
                env = new System.Collections.Generic.List<string>();
                var ec = environment;
                while (ec is Cons envCons) {
                    env.Add(envCons.Car is LispString evs ? evs.Value : envCons.Car.ToString());
                    ec = envCons.Cdr;
                }
            }
        ...
        RegisterDotNet(DotNetPkg, "CALL-OUT", new LispFunction(Runtime.DotNetCallOut, "DOTNET:CALL-OUT", -1),
            "(dotnet:call-out object \"Method\" &rest args) => value, out1, out2, ...\nInvoke an instance method that has out/ref parameters. Supply every argument\nexcept the out ones; a ref parameter is in-out, so pass its initial value.\nReturns the method's return value (T for void) as the primary value, followed\nby each out/ref argument as additional values.");
...
#endif
#if !DOTCL_NO_STA
        RegisterDotNet(DotNetPkg, "UI-INVOKE", new LispFunction(DotNetSta.UiInvoke, "DOTNET:UI-INVOKE", -1),
        ...
        RegisterDotNet(DotNetPkg, "UI-POST", new LispFunction(DotNetSta.UiPost, "DOTNET:UI-POST", -1),
        ...
        RegisterDotNet(DotNetPkg, "EXCEPTION-OBJECT", new LispFunction(args => Runtime.DotNetExceptionObject(args[0]), "DOTNET:EXCEPTION-OBJECT", 1),
            "(dotnet:exception-object condition) => exception-or-nil\nFor a condition wrapping a raw .NET exception, return the exception instance\nitself as a .NET object — e.g. to read SocketException.SocketErrorCode or walk\nthe InnerException chain. NIL for an ordinary Lisp condition.");
