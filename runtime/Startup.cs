...
public static class Startup
{
    ...
    /// <summary>A short digest of the core this image booted from, or null when it
    /// cannot be determined. A .fasl records the value current when it was
    ...
    /// effect. Computed on first use, never at startup: the core is megabytes and
    /// hashing it would show up in startup time for something only fasl
    /// compilation and loading care about.</summary>
    public static string? CoreGeneration()
    {
        ...
        try
        {
            if (CorePath != null && System.IO.File.Exists(CorePath))
            {
                using var sha = System.Security.Cryptography.SHA256.Create();
                using var fs = System.IO.File.OpenRead(CorePath);
                var hash = sha.ComputeHash(fs);
                _coreGeneration = Compat.ToHexString(hash, 0, 8).ToLowerInvariant();
            }
        }
