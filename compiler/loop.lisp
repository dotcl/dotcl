...
(defun loop-warn (format-string &rest format-args)
  ;; Signal STYLE-WARNING (not plain WARNING) so compile-file failure-p is not set
  (warn 'style-warning
        :format-control
        (format nil "~A~%Current LOOP context:~{ ~S~}."
...
(defun loop-package-symbols-iteration-path (variable data-type prep-phrases &key symbol-types)
  ...
  (let* ((list-var (loop-gentemp 'loop-pkgsym-list-))
	 (variable (or variable (loop-gentemp)))
	 (pkg (or (cadar prep-phrases) '*package*))
	 ;; Choose the right runtime function based on symbol-types.
	 ;; We collect all matching symbols into a list upfront, then iterate.
	 ...
	 (collector (cond ((or (equal symbol-types '(:internal :external :inherited))
			  ...
			  ((or (equal symbol-types '(:internal :external))
			   ...
			   ;; present symbols (internal + external, no inherited).
			   ;; Filter %package-all-symbols to exclude :inherited.
			   (let ((s-var (loop-gentemp 'loop-pkgsym-s-))
				 (r-var (loop-gentemp 'loop-pkgsym-r-))
				 (st-var (loop-gentemp 'loop-pkgsym-st-))
				 (ig-var (loop-gentemp 'loop-pkgsym-ig-))
				 (p-var (loop-gentemp 'loop-pkgsym-p-)))
			     `(let ((,r-var nil)
				    (,p-var (find-package ,pkg)))
				(dolist (,s-var (%package-all-symbols ,p-var) ,r-var)
				  (multiple-value-bind (,ig-var ,st-var)
				      (find-symbol (symbol-name ,s-var) ,p-var)
				    (declare (ignore ,ig-var))
				    (when (or (eq ,st-var :internal) (eq ,st-var :external))
				      (push ,s-var ,r-var)))))))
			  (t
			   ;; fallback: all symbols
			   `(%package-all-symbols ,pkg)))))
    `(((,variable nil ,data-type) (,list-var ,collector))
      ()
      (endp ,list-var)
      (,variable (car ,list-var) ,list-var (cdr ,list-var))
      ()
      ())))
...
(defun loop-string-codepoint-iteration-path (variable data-type prep-phrases)
  (cond ((or (cdr prep-phrases) (not (member (caar prep-phrases) '(:in :of))))
	...
	((null prep-phrases) (loop-error "Missing OF or IN in ~S iteration path.")))
  (unless (symbolp variable)
    (loop-error "Destructuring is not valid for string codepoint iteration."))
  (let ((string-var (loop-gentemp 'loop-codepoint-string-))
    ...
    (let* ((val-var nil)
      ...
      (push `(with-string-codepoint-iterator (,next-fn ,string-var)) *loop-wrappers*)
...
(defun loop-string-glyph-string-iteration-path (variable data-type prep-phrases)
  (cond ((or (cdr prep-phrases) (not (member (caar prep-phrases) '(:in :of))))
	...
	((null prep-phrases) (loop-error "Missing OF or IN in ~S iteration path.")))
  (unless (symbolp variable)
    (loop-error "Destructuring is not valid for string glyph iteration."))
  (let ((string-var (loop-gentemp 'loop-glyph-string-))
    ...
    (let* ((val-var nil)
	   ...
	   (bindings `((,variable nil ,data-type)
		       (,string-var ,(cadar prep-phrases)))))
      (push `(with-string-glyph-iterator (,next-fn ,string-var)) *loop-wrappers*)
      (setq val-var variable)
      `(,bindings			;bindings
	()				;prologue
	()				;pre-test
...
(defun make-ansi-loop-universe (extended-p)
  (let ((w (make-standard-loop-universe
    ...
    (add-loop-path '(element elements) 'loop-sequence-elements-path w
		   :preposition-groups '((:of :in))
		   :inclusive-permitted nil
		   :user-data '(:fetch-function elt :size-function length))
    (when extended-p
...
;;; ================================================================
;;; Internal loop macro expanders as defuns (compiled to CIL for TARGET)
;;; ================================================================
...
(defun %expand-with-loop-list-collection-head (form)
  ;; (with-loop-list-collection-head (head tail [user-head]) body...)
  (let* ((args (cadr form))
         (head-var (first args))
         (tail-var (second args))
         (user-head-var (third args))
         (body (cddr form))
         (l (and user-head-var (list (list user-head-var nil)))))
    `(let* ((,head-var (list nil)) (,tail-var ,head-var) ,@l)
       ,@body)))

(defun %expand-loop-collect-rplacd (form)
  ;; (loop-collect-rplacd (head tail [user-head]) collect-form)
  (let* ((args (cadr form))
         (head-var (first args))
         (tail-var (second args))
         (user-head-var (third args))
         (collected-form (caddr form)))
    ;; macroexpand with nil env (safe: only affects optimization)
    (setq collected-form (macroexpand collected-form nil))
    (flet ((cdr-wrap (form n)
             (do () ((<= n 4)
                     (setq form `(,(case n (1 'cdr) (2 'cddr) (3 'cdddr) (4 'cddddr))
                                  ,form)))
               (setq form `(cddddr ,form) n (- n 4)))))
      (let ((tail-form collected-form) (ncdrs nil))
        (when (consp collected-form)
          (cond ((eq (car collected-form) 'list)
                 (setq ncdrs (1- (length (cdr collected-form)))))
                ((member (car collected-form) '(list* cons))
                 (when (and (cddr collected-form) (member (car (last collected-form)) '(nil 'nil)))
                   (setq ncdrs (- (length (cdr collected-form)) 2))))))
        (let ((answer
                (cond ((null ncdrs)
                       `(when (setf (cdr ,tail-var) ,tail-form)
                          (setq ,tail-var (last (cdr ,tail-var)))))
                      ((< ncdrs 0) (return-from %expand-loop-collect-rplacd nil))
                      ((= ncdrs 0)
                       `(rplacd ,tail-var (setq ,tail-var ,tail-form)))
                      (t `(setq ,tail-var ,(cdr-wrap `(setf (cdr ,tail-var) ,tail-form)
                                                     ncdrs))))))
...
(defun %expand-loop-collect-answer (form)
  ;; (loop-collect-answer head-var [user-head-var])
  (let ((head-var (cadr form))
        (user-head-var (caddr form)))
    (or user-head-var
        `(cdr ,head-var))))

(defun %expand-with-minimax-value (form)
  ;; (with-minimax-value lm body...)
  (let* ((lm (cadr form))
         (body (cddr form))
         (init (loop-typed-init (loop-minimax-type lm)))
         (which (car (loop-minimax-operations lm)))
         (infinity-data (loop-minimax-infinity-data lm))
         (answer-var (loop-minimax-answer-variable lm))
         (temp-var (loop-minimax-temp-variable lm))
...
(defun %expand-loop-accumulate-minimax-value (form)
  ;; (loop-accumulate-minimax-value lm operation value-form)
  (let* ((lm (cadr form))
         (operation (caddr form))
         (value-form (cadddr form))
         (answer-var (loop-minimax-answer-variable lm))
         (temp-var (loop-minimax-temp-variable lm))
         (flag-var (loop-minimax-flag-variable lm))
         (test (hide-variable-reference t answer-var
                 `(,(ecase operation (min '<) (max '>))
                   ,temp-var ,answer-var))))
    `(progn
       (setq ,temp-var ,value-form)
       (when ,(if flag-var `(or (not ,flag-var) ,test) test)
         (setq ,@(and flag-var `(,flag-var t))
               ,answer-var ,temp-var)))))

(defun %expand-loop-store-table-data (form)
  ;; (loop-store-table-data symbol table datum)
  (let ((symbol (cadr form))
        (table (caddr form))
        (datum (cadddr form)))
    `(setf (gethash (symbol-name ,symbol) ,table) ,datum)))

(defun %expand-loop-really-desetq (form)
  ;; (loop-really-desetq var1 val1 var2 val2 ...)
  (let ((var-val-pairs (cdr form)))
    (labels ((find-non-null (var)
               (do ((tail var)) ((not (consp tail)) tail)
                 (when (find-non-null (pop tail)) (return t))))
             (loop-desetq-internal (var val &optional temp)
                    ...
                    (if (eq (car val) 'prog1)
                        (mapcan (lambda (x)
                                  (and (consp x)
                                       (or (not (eq (car x) 'car))
                                           (not (symbolp (cadr x)))
                                           (not (symbolp (setq x (macroexpand x nil)))))
                                       (cons x nil)))
                                (cdr val))
                        `(,val))))
                 ((consp var)
...
(defun %expand-loop-body (form)
  ;; (loop-body prologue before-loop main-body after-loop epilogue)
  ;; Pre-expand internal loop macros in all sublists so that
  ;; estimate-code-size only sees standard CL forms (no dotted-pair
  ;; destructuring args from LOOP-REALLY-DESETQ, etc.)
  (let ((prologue (mapcar #'loop-expand-all (cadr form)))
        (before-loop (mapcar #'loop-expand-all (caddr form)))
        (main-body (mapcar #'loop-expand-all (cadddr form)))
        (after-loop (mapcar #'loop-expand-all (car (cddddr form))))
        (epilogue (mapcar #'loop-expand-all (cadr (cddddr form))))
        (rbefore nil) (rafter nil) (flagvar nil))
    (unless (= (length before-loop) (length after-loop))
      (error "LOOP-BODY called with non-synched before- and after-loop lists."))
    (setq rbefore (reverse before-loop) rafter (reverse after-loop))
    (labels ((psimp (l)
             ...
             (makebody ()
               ...
               (let ((result `(tagbody
                              ...
                              next-loop
                                ...
                                ,@(psimp epilogue))))
                 (if flagvar `(let ((,flagvar nil)) ,result) result))))
      (when (or *loop-duplicate-code* (not rbefore))
        (return-from %expand-loop-body (makebody)))
      ;; Threshold is always 40 (loop-optimization-quantities returns (1 1 1 1 1))
      (do ((threshold 40)) (nil)
        (do () ((or (null rbefore) (not (equal (car rbefore) (car rafter)))))
          (push (pop rbefore) main-body)
          (pop rafter))
        (unless rbefore (return (makebody)))
        (do ((bb rbefore (cdr bb)) (aa rafter (cdr aa)) (lastdiff nil) (count 0) (inc nil))
            ((null bb) (return-from %expand-loop-body (makebody)))
          (cond ((not (equal (car bb) (car aa))) (setq lastdiff bb count 0))
                ((or (not (setq inc (estimate-code-size (car bb) nil)))
                     (> (incf count inc) threshold))
                 (let ((then nil) (else nil))
                   (do () (nil)
                     (push (pop rbefore) else)
...
;;; Set of internal loop macros that must be expanded before
;;; returning to the dotcl compiler.
(defvar *loop-internal-macros*
  '("LOOP-COPYLIST*" "WITH-LOOP-LIST-COLLECTION-HEAD"
    "LOOP-COLLECT-RPLACD" "LOOP-COLLECT-ANSWER"
    "WITH-MINIMAX-VALUE" "LOOP-ACCUMULATE-MINIMAX-VALUE"
    "LOOP-STORE-TABLE-DATA" "LOOP-REALLY-DESETQ" "LOOP-BODY"))

(defun loop-internal-macro-p (sym)
  "Check if SYM is an internal loop macro that needs expansion."
  (and (symbolp sym)
       (member (symbol-name sym) *loop-internal-macros* :test #'string=)))

;;; Register internal loop macros in *macros* for both HOST and TARGET
(setf (gethash 'loop-copylist* dotcl.cil-compiler::*macros*) #'%expand-loop-copylist*)
(setf (gethash 'with-loop-list-collection-head dotcl.cil-compiler::*macros*) #'%expand-with-loop-list-collection-head)
(setf (gethash 'loop-collect-rplacd dotcl.cil-compiler::*macros*) #'%expand-loop-collect-rplacd)
(setf (gethash 'loop-collect-answer dotcl.cil-compiler::*macros*) #'%expand-loop-collect-answer)
(setf (gethash 'with-minimax-value dotcl.cil-compiler::*macros*) #'%expand-with-minimax-value)
(setf (gethash 'loop-accumulate-minimax-value dotcl.cil-compiler::*macros*) #'%expand-loop-accumulate-minimax-value)
(setf (gethash 'loop-store-table-data dotcl.cil-compiler::*macros*) #'%expand-loop-store-table-data)
(setf (gethash 'loop-really-desetq dotcl.cil-compiler::*macros*) #'%expand-loop-really-desetq)
(setf (gethash 'loop-body dotcl.cil-compiler::*macros*) #'%expand-loop-body)

(defun loop-expand-all (form)
  "Recursively expand only internal loop macros in FORM.
   Uses *macros* hash table (works on both HOST and TARGET).
  ...
  (cond
    ...
    ((and (loop-internal-macro-p (car form))
     ...
     (loop-expand-all
      (funcall (dotcl.cil-compiler::find-macro-expander (car form)) form)))
    (t
     ;; Walk into the list, but preserve structure if nothing changes
     (let ((changed nil)
           (result '())
           (current form))
       (loop while (consp current)
             do (let* ((orig (car current))
                       (expanded (loop-expand-all orig)))
                  (unless (eq orig expanded) (setq changed t))
       ...
       (if (and (not changed) (null current))
           ...
           (let ((proper (nreverse result)))
             (if (null current)
                 proper
                 (let ((last-cons (last proper)))
                   (if last-cons
                       (progn (rplacd last-cons (loop-expand-all current))
                              proper)
                       (loop-expand-all current))))))))))

;;; Register LOOP in dotcl's macro table
(setf (gethash 'loop dotcl.cil-compiler::*macros*)
      (lambda (form)
        (loop-expand-all
         (loop-standard-expansion (cdr form) nil *loop-ansi-universe*))))

;;; Register LOOP-FINISH in dotcl's macro table
(setf (gethash 'loop-finish dotcl.cil-compiler::*macros*)
      (lambda (form)
        (declare (ignore form))
        '(go end-loop)))
